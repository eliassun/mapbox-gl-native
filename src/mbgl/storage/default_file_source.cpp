...
void DefaultFileSource::Impl::cancel(Request* req) {
    ...
    if (request) {
        // If the number of dependent requests of the DefaultFileRequest drops to zero, the
        // unsubscribe callback triggers the removal of the DefaultFileRequest pointer from the list
        ...
        if (request->observers.empty()) {
            ...
            // First, remove this DefaultFileRequest from the source.
